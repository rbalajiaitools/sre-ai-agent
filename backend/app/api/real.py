...
async def export_investigation_postmortem(investigation_id: str, db: AsyncSession = Depends(get_db)):
    """Export investigation as a post-mortem document (Markdown format)."""
    from fastapi.responses import Response
    
    try:
        logger.info("exporting_investigation", investigation_id=investigation_id)
        ...
        # Generate post-mortem document in Markdown format
        postmortem = f"""# Post-Mortem Report
## Incident: {investigation.incident_number}

**Service:** {investigation.service_name or 'N/A'}
**Status:** {investigation.status}
**Started:** {investigation.started_at.strftime('%Y-%m-%d %H:%M:%S UTC')}
**Completed:** {investigation.completed_at.strftime('%Y-%m-%d %H:%M:%S UTC') if investigation.completed_at else 'In Progress'}
**Duration:** {(investigation.completed_at - investigation.started_at).total_seconds() / 60:.1f} minutes
"""

        if investigation.approved_by:
            postmortem += f"""**Approved By:** {investigation.approved_by}
**Approved At:** {investigation.approved_at.strftime('%Y-%m-%d %H:%M:%S UTC') if investigation.approved_at else 'N/A'}
"""
        ...
        if investigation.rca:
            rca = investigation.rca
            postmortem += f"""## Root Cause Analysis
...
"""
            ...
            if rca.get('affected_resources'):
                postmortem += "### Affected Resources\n"
                for resource in rca['affected_resources']:
                    postmortem += f"- {resource}\n"
                postmortem += "\n"
            
            if rca.get('contributing_factors'):
                postmortem += "### Contributing Factors\n"
                for i, factor in enumerate(rca['contributing_factors'], 1):
                    postmortem += f"{i}. {factor}\n"
                postmortem += "\n"
            
            if rca.get('incident_timeline'):
                postmortem += "### Incident Timeline\n"
                for event in rca['incident_timeline']:
                    postmortem += f"- **{event.get('timestamp')}** [{event.get('severity')}] {event.get('event')} (Source: {event.get('source')})\n"
                postmortem += "\n"
        
        # Add Resolution if available
        if investigation.resolution:
            resolution = investigation.resolution
            postmortem += f"""## Resolution

### Recommended Fix
{resolution.get('recommended_fix', 'N/A')}

### Steps to Resolve
"""
            for i, step in enumerate(resolution.get('fix_steps', []), 1):
                postmortem += f"{i}. {step}\n"
            ...
            if resolution.get('estimated_impact'):
                postmortem += f"### Estimated Impact\n{resolution['estimated_impact']}\n\n"
        
        # Add Agent Results
        if investigation.agent_results:
            postmortem += "## Investigation Details\n\n"
            for agent in investigation.agent_results:
                postmortem += f"### {agent.get('agent_name', 'Unknown Agent')}\n"
                postmortem += f"**Status:** {agent.get('status', 'unknown')}\n"
                if agent.get('findings'):
                    postmortem += f"**Findings:** {agent['findings']}\n"
                postmortem += "\n"
        
        postmortem += "\n---\n\n"
        postmortem += f"*Generated by CloudScore Astra AI on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}*\n"
        
        logger.info("investigation_exported", investigation_id=investigation_id)
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=investigation-{investigation_id}-postmortem.md"
            }
        )
        ...
        logger.error("export_investigation_error", investigation_id=investigation_id, error=str(e))
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/investigations/{investigation_id}/approve-resolution")
async def approve_investigation_resolution(
    investigation_id: str,
    request: dict,
    db: AsyncSession = Depends(get_db)
):
    """Approve resolution and close ServiceNow ticket."""
    try:
        ...
        logger.info("approving_resolution", investigation_id=investigation_id, approved_by=approved_by)
        ...
        if investigation.status != "rca_complete":
            raise HTTPException(
                status_code=400,
                detail=f"Investigation must be in rca_complete status. Current status: {investigation.status}"
            )
        
        if not investigation.resolution:
            raise HTTPException(status_code=400, detail="No resolution available to approve")
        
        # Update investigation with approval details
        investigation.approved_by = approved_by
        ...
        # Try to update ServiceNow ticket if integration exists
        try:
            tenant_id = investigation.tenant_id
            sn_integrations = await crud.get_integrations(db, tenant_id, "servicenow")
            
            if sn_integrations and investigation.incident_number:
                connector = get_servicenow_connector()
                
                # Create work note with resolution
                resolution = investigation.resolution
                work_note = f"""Resolution Approved by {approved_by}
...
"""
                ...
                # Get incident from ServiceNow using incident number directly
                # Note: tenant_id might not be a valid UUID, so we'll use the connector's method
                # that doesn't require UUID conversion
                incidents = await connector.get_incidents(
                    tenant_id=tenant_id,  # Pass as string, connector will handle it
                    limit=100
                )
                ...
                for inc in incidents:
                    if inc.number == investigation.incident_number:
                        incident = inc
                        break
                
                if incident:
                    # Add work note
                    await connector.client.add_work_note(incident.sys_id, work_note)
                    
                    # Close the incident
                    await connector.client.update_incident(
                        incident.sys_id,
                        {
                            "state": "6",  # Resolved state in ServiceNow
                            "close_code": "Solved (Permanently)",
                            "close_notes": f"Resolved by CloudScore Astra AI. Approved by {approved_by}."
                        }
                    )
                    
                    logger.info("servicenow_ticket_closed", 
                               investigation_id=investigation_id,
                               incident_number=investigation.incident_number)
                else:
                    logger.warning("incident_not_found_in_servicenow",
                                  investigation_id=investigation_id,
                                  incident_number=investigation.incident_number)
        except Exception as sn_error:
            logger.warning("servicenow_update_failed", 
                          investigation_id=investigation_id,
                          error=str(sn_error))
            # Don't fail the approval if ServiceNow update fails
        
        logger.info("resolution_approved", investigation_id=investigation_id, approved_by=approved_by)
        ...
        return {
            ...
            "status": investigation.status,
            "approved_by": investigation.approved_by,
            "approved_at": investigation.approved_at.isoformat(),
        ...
        logger.error("approve_resolution_error", investigation_id=investigation_id, error=str(e))
        await db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
